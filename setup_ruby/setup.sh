...
if [ $# -ne 1 ]
then
  echo 'Usage: ./setup.sh "Nome do Problema"'
  exit 1
fi
...
if [ -d "$folder" ]; then
  ...
  [ ! -d "$folder" ] && mkdir -p "$folder" && cp -r "$(pwd)/" "$folder/"

  echo ">>> Set date"
  sed "s/\[DATA\]/$(date "+%Y-%m-%d %H:%M:%S")/g" "README.md" > "$folder/README_tmp.md"

  echo ">>> Set Problem"
  sed "s/\\[PROBLEM\\]/$problem/g" "$folder/README_tmp.md" > "$folder/README.md"

  echo ">>> Set Ruby Version"
  sed "s/\[RUBY_VERSION\]/$(cat .ruby-version)/g" "$folder/README.md" > "$folder/README_tmp.md"
